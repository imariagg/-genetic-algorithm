...
def iteraciones(poblacion_reemplazada, si_hay_referencia, valor_fitness_referencia, max_iteraciones_sin_mejorar, si_hay_max_iteraciones, max_iteraciones, maximiza_minimiza): #Si no hay max iteraciones se pone a -1
    ...
    while(cont_iteraciones_sin_mejorar != max_iteraciones_sin_mejorar or it==max_iteraciones ):
        ...
        poblacion_seleccionada=seleccionTorneo(copia_poblacion_inicial2, 50,3,1)  
        #poblacion_seleccionada=seleccionOrdenLineal(copia_poblacion_inicial2, 50, 1)
        #poblacion_seleccionada=seleccionEmparejamientoVariadoInversoNAM(copia_poblacion_inicial2, 50, 6, 'PERMUTACION')#poblacion, seleccionado, N_torneo, tipo
        #poblacion_seleccionada=seleccionRuleta(copia_poblacion_inicial2, 50, 1)
       
        poblacion_cruzada=crucePermutacionOX(poblacion_seleccionada, 30, 0.7) #poblacion, n_puntos_cruce, n_cruces, probabilidad

        poblacion_mutada=mutacionPermutacion(poblacion_cruzada,0.2,0.2) #poblacion, probabilidad_individuo, probabilidad_gen

        poblacion_reemplazada=reemplazoPeorRW(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 1, 'PERMUTACION')
        #poblacion_reemplazada=reemplazoTorneoRestringuidoTRS(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 3, 1, 'PERMUTACION')
        #poblacion_reemplazada=reemplazoPeorEntreSemejantes(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 3, 1, 'PERMUTACION')
        #poblacion_reemplazada=reemplazoCrowdingDeterministicoDC(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 1, 'PERMUTACION')
...
def generaPoblacionPermutacion(tamanoPoblacion, numeroVar):
    ...
    while(tamanoPoblacion!=len(lista_de_genes)):

        for gen in range(1,numeroVar+1):
            genes_aux.append(gen)
        
        random.shuffle(genes_aux)
        
        if genes_aux not in lista_de_genes:
            lista_de_genes.append(genes_aux)
        genes_aux=[]

    for genes in lista_de_genes:

        individuo = Individuo(genes,0)
...
def funcionObjetivo(poblacion, tipo):
    ...
    if tipo=='PERMUTACION':
        return funcionObjetivoP(poblacion)
    
def funcionObjetivoP(poblacion_inicial):
    ...
    if len(poblacion_inicial)==0:
        ...
        individuo_aux=poblacion_inicial[1]
    ...
    for individuo in poblacion_inicial:
        ...
        for gen in genes:
            cont=cont+gen
        individuo.setFitness(round((cont/genes[-1]),redondeo))
...
def distanciaP(individuo1, individuo2):
    
    total_posibles_transiciones = len(individuo1.getGenes())
    transiciones1 = []
    transiciones2 = []
    n_transiciones = 0

    for i in range(0, total_posibles_transiciones):
        if i == total_posibles_transiciones - 1:
            transicion = [individuo1.getGenes()[-1], individuo1.getGenes()[0]]
        else:
            transicion = [individuo1.getGenes()[i], individuo1.getGenes()[i+1]]
        transiciones1.append(transicion)

    for i in range(0, total_posibles_transiciones):
        if i == total_posibles_transiciones - 1:
            transicion = [individuo2.getGenes()[-1], individuo2.getGenes()[0]]
        else:
            transicion = [individuo2.getGenes()[i], individuo2.getGenes()[i+1]]
        transiciones2.append(transicion)

    for i in range(0, len(transiciones1)):
        for j in range(0, len(transiciones1)):
            if transiciones1[i] == transiciones2[j]:
                n_transiciones = n_transiciones + 1

    distancia = total_posibles_transiciones - n_transiciones
...
def distancia(individuo1, individuo2, tipo):
    ...
    if tipo=='PERMUTACION':
        return distanciaP(individuo1, individuo2)
...
def crucePermutacionOX(poblacion, n_cruces, probabilidad):

    print('Poblacion antes de cruce')
    print(len(poblacion))
    poblacion_aux=poblacion
    cruzados=[]
    ...
    for k in range(0, n_cruces):
        ...
        if probabilidad>probabilidad_aux:
                padre1=random.choices(poblacion_aux)[0]
                poblacion_aux.remove(padre1)
                padre2=random.choices(poblacion_aux)[0]
                poblacion_aux.append(padre1)
                
                punto_corte1=random.randint(1,len(padre1.getGenes())-1)
                punto_corte2=random.randint(1,len(padre1.getGenes())-1)
        

                while(punto_corte1==punto_corte2):
                    punto_corte2=random.randint(1,len(poblacion_aux)-1)

                if punto_corte1>punto_corte2:
                    punto_corte1, punto_corte2 = punto_corte2, punto_corte1

                
                rango=padre1.getGenes()[punto_corte1:punto_corte2]

                hijo_genes=[]
                
                for i in range(0,len(rango)):
                    hijo_genes.append(rango[i])
                    
                  
                padre2_aux=Individuo([],0)
    
                
                for i in range(0,len(padre2.getGenes())):
                    if padre2.getGenes()[i] not in rango:
                             
                        padre2_aux.getGenes().append(padre2.getGenes()[i])
                        
               
                it=0

                for k in range(punto_corte2,len(padre2.getGenes())): #puede que sea punto_corte-1
                    hijo_genes.insert(k,padre2_aux.getGenes()[it])
                    it=it+1
          
                
                for k in range(0,punto_corte1):
                    hijo_genes.insert(k,padre2_aux.getGenes()[it])
                    it=it+1

                hijo=Individuo(hijo_genes,0, padre1.getGenes(), padre2.getGenes())
                cruzados.append(hijo)
          
    print('Cruzados')
    print(len(cruzados))
    funcionObjetivoP(cruzados)
    return cruzados

#--------------------------------------------MUTACIÓN---------------------------------------------------------------------------------

def mutacionPermutacion(poblacion, probabilidad_individuo, probabilidad_gen):
        ...
        if probabilidad_individuo_aux < probabilidad_individuo:
            ...
            for it in range(len(individuo.getGenes())): 
                ...
                if probabilidad_gen_aux < probabilidad_gen:
                    ...
                    gen_a=individuo.getGenes()[it]
                    posicion_del_gen_b=random.randint(1,len(individuo.getGenes())-1)
                    gen_b=individuo.getGenes()[posicion_del_gen_b]
                    
                    while(gen_a==gen_b):
                        posicion_del_gen_b=random.randint(1,len(individuo.getGenes())-1)
                        gen_b=individuo.getGenes()[posicion_del_gen_b]
                    
                    individuo.getGenes()[it]=gen_b
                    individuo.getGenes()[posicion_del_gen_b]=gen_a
                    


    print('Mutados')
    print(len(poblacion_aux))
    funcionObjetivoP(poblacion_aux)
    return poblacion_aux  
...
def reemplazoPeorRW(p_inicial, p_mutada, elitismo, maximiza_miniza, tipo):
    ...
    p_elitista=[]
    p_reemplazada=[]
    #Sacar del porcentaje de elitismo cuantos individuos serian, y de este sacar a los mejores
...
poblacion_inicial=funcionObjetivo(generaPoblacionPermutacion(100,10), 'PERMUTACION') 
...
#poblacion_seleccionada=seleccionEmparejamientoVariadoInversoNAM(poblacion_inicial, 50, 6,'PERMUTACION') #poblacion, seleccionado, N_torneo, tipo
#poblacion_seleccionada=seleccionRuleta(poblacion_inicial, 50, 1)

poblacion_cruzada=crucePermutacionOX(poblacion_seleccionada, 30, 0.7) #poblacion, n_cruces, probabilidad

poblacion_mutada=mutacionPermutacion(poblacion_cruzada,0.2,0.2) #poblacion, probabilidad_individuo, probabilidad_gen

poblacion_reemplazada=reemplazoPeorRW(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 1, 'PERMUTACION') #p_inicial, p_mutada, elitismo, maximiza_miniza, tipo
#poblacion_reemplazada=reemplazoTorneoRestringuidoTRS(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 3, 1, 'PERMUTACION')
#poblacion_reemplazada=reemplazoPeorEntreSemejantes(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 3, 1, 'PERMUTACION')
#poblacion_reemplazada=reemplazoCrowdingDeterministicoDC(copia_poblacion_inicial_para_reemplazo, poblacion_mutada, 10, 1, 'PERMUTACION')
